...
export function TeamManager({ teams }: { teams: Team[] }) {
  ...
  return <div className="league-manager"><form action={createAction} className="team-create"><label>Team name<input name="name" required placeholder="e.g. Moonshots" /></label><label>Captain email(s)<input name="ownerEmail" required placeholder="captain@example.com, co-captain@example.com" /></label><button className="button" type="submit" disabled={creating}>{creating ? "Creating…" : "Add team"}</button><Status state={createState} /></form><div className="team-list">{teams.map((team) => <TeamCard key={team.id} team={team} />)}</div></div>;
}
...
function TeamCard({ team }: { team: Team }) {
  ...
  return <article className="team-admin-card"><form action={updateAction} className="team-edit"><input type="hidden" name="teamId" value={team.id} /><label>Team name<input name="name" defaultValue={team.name} required /></label><label>Captain email(s)<input name="ownerEmail" defaultValue={team.ownerEmail} required /></label><p className="updated">Comma-separate multiple captain emails.</p><p className="updated">URL: /teams/{team.slug} · {team.slots.length} slots</p><button className="button" type="submit" disabled={updating}>{updating ? "Saving…" : "Save changes"}</button><Status state={updateState} /></form><form action={deleteAction} onSubmit={(event) => { if (!window.confirm(`Delete ${team.name} and its 12 empty roster slots?`)) event.preventDefault(); }}><input type="hidden" name="teamId" value={team.id} /><button className="button button-danger" type="submit" disabled={locked || deleting}>{locked ? "Has league history" : deleting ? "Deleting…" : "Delete team"}</button><Status state={deleteState} /></form></article>;
}
